...
const DRY_RUN = true; // set to false only after a successful dry run
const MAX_SMS = 100;
...
async function runOnce() {
  const clientMongo = new MongoClient(MONGODB_URI, { serverSelectionTimeoutMS: 10000 });
  try {
    ...
    const ALLOWED_SOURCES = ["Fundraising App", "Website"];

    const cursor = donationsColl.find({
      StageName: "Closed Won",
      Donation_Source__c: { $in: ALLOWED_SOURCES },
      smsSent: { $ne: true },
      CloseDate: { $gte: todayStart, $lte: todayEnd },
    });

    const matchCount = await donationsColl.countDocuments({
      StageName: "Closed Won",
      Donation_Source__c: { $in: ALLOWED_SOURCES },
      smsSent: { $ne: true },
      CloseDate: { $gte: todayStart, $lte: todayEnd },
    });
    console.log(`[INFO] Query matched ${matchCount} donation(s) in the last 1 day.`);
    ...
    while (await cursor.hasNext()) {
      ...
      const latestDonation = await donationsColl.findOne(
        { contact: donation.contact, StageName: "Closed Won", Donation_Source__c: { $in: ["Fundraising App", "Website"] } },
        { sort: { CloseDate: -1 } }
      );
